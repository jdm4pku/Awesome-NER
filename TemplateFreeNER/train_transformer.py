...
from transformers import CONFIG_MAPPING,MODEL_MAPPING,SchedulerType
from transformers import AutoConfig, AutoTokenizer,AutoModelForMaskedLM
from transformers import XLNetLMHeadModel
from transformers import default_data_collator,DataCollatorForTokenClassification,get_scheduler
...
def parser_args():
    ...
    parser.add_argument("--train_file", type=str, default='', help="A csv or a json file containing the training data.")
    parser.add_argument("--validation_file", type=str, default='', help="A csv or a json file containing the validation data.")
    parser.add_argument("--dev_file", type=str, default=None, help="A csv or a json file containing the validation data.")
    ...
    parser.add_argument("--model_name_or_path",type=str,help="Path to pretrained model or model identifier from huggingface.co/models.", default='bert-base-cased')
    ...
    parser.add_argument("--label_schema",default='IO',help="Activate debug mode and run training only with a subset of data.",)
    parser.add_argument("--eval_label_schema",default='BIO',help="Activate debug mode and run training only with a subset of data.",)
    parser.add_argument("--label_map_path",default=None,type=str,help="label map path",)
    parser.add_argument("--do_crf",action="store_true",)
    parser.add_argument("--crf_raw_path",default=None,type=str)
...
def add_label_token_bert(model, tokenizer, label_map):
    sorted_add_tokens = sorted(list(label_map.keys()), key=lambda x: len(x), reverse=True)
    tokenizer.add_tokens(sorted_add_tokens)
    num_tokens, _ = model.bert.embeddings.word_embeddings.weight.shape # 词汇表的大小 * embed_size
    model.resize_token_embeddings(len(sorted_add_tokens)+num_tokens)
    for token in sorted_add_tokens:
        if token.startswith('B-') or token.startswith('I-'):  # 特殊字符
            index = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(token))
...
def main():
    ...
    if args.label_map_path is not None:
        ...
        ori_label_token_map = {
            "I-PER":['Michael', 'John', 'David', 'Mark', 'Martin', 'Paul'], 
            "I-ORG": ['Inc', 'Co', 'Corp', 'Party', 'Association', '&'],
            "I-LOC":['Germany', 'Australia', 'England', 'France', 'Italy', 'Belgium'], 
    ...
    if args.model_name_or_path:
        if "xlnet" in args.model_name_or_path:
            model = XLNetLMHeadModel.from_pretrained(args.model_name_or_path,from_tf=bool(".ckpt" in args.model_name_or_path),config=config,)
        else:
            model = AutoModelForMaskedLM.from_pretrained(args.model_name_or_path,from_tf=bool(".ckpt" in args.model_name_or_path),config=config,)
    else:
    ...
    # Metrics
    metric = load_metric("TemplateFreeNER/seqeval_metric.py")
    ...
    def evaluate(best_metric, load=False, use_crf=False):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            predictions = outputs.logits.argmax(dim=-1) # 预测值
            labels = ner_label  # 真实值
            token_labels = batch.pop("input_ids") # 输入值
            if use_crf:
                ...
                # labels = labels.cpu()
                # token_labels =token_labels.cpu()
                # emissions = emissions.cpu()
                preds, refs, tokens = get_labels(predictions, labels, token_labels, emissions, viterbi_decoder)
            else:
                preds, refs, tokens = get_labels(predictions, labels, token_labels)
        ...
        if best_metric == -1 or best_metric["overall_f1"] < eval_metric["overall_f1"] and not load:
            ...
            if args.output_dir is not None:
                ...
                model.save_pretrained(args.output_dir)  # 直接使用模型的save_pretrained方法
                tokenizer.save_pretrained(args.output_dir)  # tokenizer的保存方法不变
