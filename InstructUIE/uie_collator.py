...
class DataCollatorForUIE:
    ...
    def decoder_call(self, batch, return_tensors):
        ...
        if self.text_only:
            ...
            # 这是一个与输入序列相同长度的布尔掩码，用于指示哪些位置是实际的有效输入，哪些位置是填充的
            label_mask = model_inputs["attention_mask"].bool()
            # 创建一个新的键，这个键对应的值是通过masked fill函数生成的
            # masked fill是对attention_mask为True位置的input_ids替换为指定的值。
            # 因为这里取非，attention_mask为False的位置的input_ids替换为label_pad_token_id，这样模型在计算损失时就不会考虑这些填充的位置
