...
def main():
    ...
    model.resize_token_embeddings(len(tokenizer)) # tokenizer可能会加入一些领域特定的词汇，因此模型的嵌入层需要被调整，以适应新的词汇表的大小
    # max_position_embeddings表示模型能够处理的的最大序列长度
    if (
    ...
    # 如果模型没有preprare_decoder_input_ids_from_labels可能意味着不是一个序列到序列的模型
    ...
    # 为了在损失计算中忽略填充标记，一个常见的做法是将填充标记的目标标签设置为 -100。
    ...
    data_collator = DataCollatorForUIE(
        ...
        pad_to_multiple_of=8 if training_args.fp16 else None, #序列的长度将被填充到8的倍数
    ...
    if training_args.gradient_checkpointing:
        model.gradient_checkpointing_enable() # 这个方法的作用是启用模型的梯度检查点功能。一旦调用这个方法，模型在训练时就会使用梯度检查点来减少内存使用。
    trainer = UIETrainer(
        model=model,
        args=training_args,
